/*
 * Copyright (C) 2006, 2007, 2008, 2026  Adrian Wise
 *
...
static void process_line(char i_buffer[], char o_buffer[])
{
  ...
  if (i_buffer[ib] == '*') {
    ...
    for (instr_count=0; instr_count<2; instr_count++) {
      ...
      const InstrTable::Instr *instr = instr_table->lookup(word);
      
      if (instr) {
        is_instr = true;
        InstrTable::Instr::INSTR_TYPE type = instr->type;
        if (type == InstrTable::Instr::MR)
          is_mr = operands = true;
        else if ((type == InstrTable::Instr::SH) ||
                 (type == InstrTable::Instr::IO) ||
                 (type == InstrTable::Instr::IG))
          operands = true;
      } else {
        /* Is it a pseudo-op ? */
        int k = 0;
