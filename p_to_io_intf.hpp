...
#define DEFINE_UNEXPECTED_INA(ClassName) IoStatus ClassName::ina(uint16_t instr, int16_t &data) { \
  p.anomaly(IoToPIntf::Level::ERROR, message(instr)); \
  return IoStatus::WAIT; \
}

#define DEFINE_UNEXPECTED_SKS(ClassName) IoStatus ClassName::sks(uint16_t instr) { \
  p.anomaly(IoToPIntf::Level::ERROR, message(instr)); \
  return IoStatus::WAIT; \
}

#define DEFINE_UNEXPECTED_OTA(ClassName) IoStatus ClassName::ota(uint16_t instr, int16_t data) { \
  p.anomaly(IoToPIntf::Level::ERROR, message(instr)); \
  return IoStatus::WAIT; \
}
