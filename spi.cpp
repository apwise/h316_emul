...
IoStatus SPI::ina(uint16_t instr, int16_t &data)
{
  bool skip = false;
  
  if (check_power()) return IoStatus::WAIT;
  ...
  return (skip) ? IoStatus::READY : IoStatus::WAIT;
}
...
IoStatus SPI::sks(uint16_t instr)
{
  bool skip = false;
...
IoStatus SPI::ota(uint16_t instr, int16_t data)
{
  bool skip = false;
